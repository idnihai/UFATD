...
class parsingNet(torch.nn.Module):
    def __init__(self, size=(288, 800), pretrained=True, backbone='50', cls_dim=(37, 10, 4),  num_classes= 4):
        ...
        self.num_classes = num_classes # number of anchor sets
        ...
        self.classification_network = torch.nn.Sequential(
            torch.nn.Linear(1800, self.num_classes),
        )

        init_weights(self.classification_network)
    

        self.cls = torch.nn.Sequential(         
            torch.nn.Linear(1800, self.total_dim), 
        )
    ...
    def forward(self, x):
        ...
        classification_result = self.classification_network(fea)


        group_cls = self.cls(fea).view(-1, *self.cls_dim)

        return group_cls , classification_result

        
        
def init_weights(branch):
    for layer in branch:
